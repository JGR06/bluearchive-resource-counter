...
- 현재 1920\*1080 사이즈가 확실한 경우에만 정상적으로 동작함(PC 해상도가 1920\*1080인 경우 앱플레이어가 자동으로 크기를 줄임)
- 런타임에 이미지 프로퍼티를 교체할 수 있다면 해상도 대응을 시도해봄직 하나 가능할지 알 수 없음
- 만약 불가능하다면 야만적인 해결책으로는 저해상도용 패치를 만드는 방법밖에 없음
