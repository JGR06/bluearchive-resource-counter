...
장비 설계도 및 전용무기 강화재료 이미지노가다까지 끝냈고, 대충 테스트 돌려보니 오인식문제도 당장 imageMax 프로그램 단에서 해결이 될 것 같다. 아래 3번은 무시

2번도 잠이 안 오는김에 미리 해뒀지만 테스트가 충분하지 않으므로 일단 내일 한번 더 본다.

BD, 교본 모두 오차범위 1~2개 이내에서 전부 카운팅되는것을 확인했다. 이제 앞으로 할 일은 아래와 같다:
1. 서치속도 개선
...
2. 플래너 사이트에서 export된 데이터에 인식한 데이터 포매팅해서 넣기
   1. 어렵진 않을텐데 귀찮긴 할듯...
   2. 따로 모듈 떼서 만들지 data쪽에 만들지 고민중
3. 장비 설계도 카운팅 이미지 노가다
   1. 이게 진짜 제일 하기 싫다 여기서도 비슷한이미지 문제가 있을거라...
   2. 이 작업 진행하면서 detailed_search쪽을 좀 수정해야 할 것이다. 학교이름만 없을 때라던가 그런 때... OCR 텍스트 오인식도 그렇고
