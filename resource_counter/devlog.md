# 230408 v1.1
전날 1600*900 이미지를 다 딴 뒤 해당 해상도에서 동작하는것을 확인했다.

이전 노트에 적어둔것처럼 해상도별 이미지를 전부 에셋으로 추가하는건 별로 하고싶지 않은 방법이다. 로드시간이 지원하는 해상도별로 N배가 되는 것도 문제지만 나중에 뭘 추가하고 수정할 때 번거로움이 극에 달할 것 같아서다.

따라서 추가 스크립트로 해상도별 이미지 교체를 하기 쉽게 해뒀다. xml을 읽어다가 rect, ROI만 가져와서 교체하고 이미지파일을 덮어씌우는 기능만 수행하는 간단한 스크립트다.

해상도별로 에셋 속성을 바꿔야하는 경우(유사도 허용치나 기타등등) 그 속성들도 추가만 하면 된다. 이미지 에셋끼리 이름이 겹치면 안되는 룰이 있어서 쉽게 작업했다.

이제 다음 문제는 레터박스 문제인데 솔직히 귀찮기도 하고 앱플레이어에서 레터박스가 생기게 설정하고 돌리는사람이 있을까 해서 당장은 수정 안하기로 했다.

# 230405 v1.0
테스트를 진행했는데 문제가 몇가지 있다.
1. 파이썬 설치 과정에서 문제가 있는 경우
   - Add to PATH를 빼먹는 경우가 있다. 안 익숙한사람은 몇 번을 말해도 인식하지 못하기때문에 친구들도 몇 번씩 새로 깔았다.
   - 설치 후 PATH length limit을 풀어야 파이썬이 제대로 동작하는 경우가 있다. 이걸 설치 프로그램이 하라고 하는데 그냥 넘겨버리는 문제가 있었다.
   - 이래서 시작하기도전에 뭔가 잘 안된다는 인상을 줘버림
2. 해상도에 문제가 있는 경우
   - 사실 그냥 앱플레이어 크기를 1920\*1080 해상도로 진행시키려고 했으나, 내생각보다 아직 PC 해상도가 1920\*1080인 사람이 많은 것 같다.
   - PC 해상도가 1920\*1080이면 앱플레이어는 그거보다 더 작게, 화면에 앱플레이어의 메뉴와 툴바가 다 나오도록 리사이징한다.
   - 따라서 그 경우 인식이 원활히 되지 않는다.
   - 강제로 키워서 화면밖으로 나가게 하면 화면 밖으로 나간 부분은 인식이 되지 않는다.
   - 이러면 해당 PC 해상도를 쓰던 사람은 이 스크립트를 원활히 사용할 수 없어 범용성이 많이 떨어진다.

나머지는 문제 없었고, 카운팅되는 것 자체는 문제없이 동작했다. 다만 해상도 이슈는 좀 치명적이다. 설치하고 파이썬 까는동안 이미 화가 나 있을텐데 해상도 문제로 스크립트까지 동작하지 않는다면 이 무슨 뻘짓인가?

1600 * 900 정도의 해상도 정도는 지원해주는게 맞는 것 같은데... 여기서 문제
1. 런타임에 ImageMax 에셋 프로퍼티를 수정할 수 있는가(파일 경로, xml로 로드하는 이미지 프로퍼티)
   - 안되면 야만적인 해결책은 해상도별 패치가 있겠다만 진짜 그러면 너무...
2. 다 돼서 수정했다 친다면 나는 1600*900 기준의 이미지도 한땀한땀 모두 따야하는가
   - 이게 제일 끔찍하다... 이미지 따는 노가다가 가장 싫은 작업이었는데...

파이썬 설치과정에서 자꾸 빼먹는건 스크린샷으로 상세히 알려준다 치는데, 해상도는 저게 가능한지 모르겠으니 당분간은 스탑이다.

일단 몇가지 편의기능은 추가했다. 수량이 급격히 바뀐경우에는 경고파일, 이전 데이터와의 수량비교를 위한 파일을 추가로 출력한다.

### 노트

화면비에 따른 해상도 변경을 처리한다면 ROI 오차는 허용범위 이내일 것이다.

그렇다면 ROI는 프로그램에서 자체적으로 찾아서 수행하고 accuracy와 이미지 두 개만 바꿔친다면 asset단위까지 갈 정도로 메타데이터는 필요가 없다.

그러므로 해상도별 이미지를 전부 asset으로 추가하는건 너무 비합리적인 선택이다. 설정을 왜 여러번 해야하는가? 리소스만 바꿔치는게 맞지 않는가?

런타임에 수정할 방법이 없다면 이미지 파일 교체하는 스크립트를 짜서 .bat 파일을 실행하라고 안내하는게 맞지 않을까 싶다

ImageMax가 이미지를 사용하기 전까지 메모리에 로드하지 않는다면 아예 이미지맥스 파이썬 스크립트에서 교체해버릴수도 있다.

지금 가장 합리적인 해결책은 수치 등의 문제는 코드에서 해결하고 이미지는 스탠드얼론 스크립트로 교체하는 것이다. 혹은 수치 등의 문제도 해당 스크립트에서 고쳐버리고 상수 등의 수치만 코드에서 해결하는게 나을 수도 있다.
